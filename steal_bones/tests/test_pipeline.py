...
def test_dynamic_collection_discovery():
    """
    Проверяет, что при нехватке кошельков запускается динамический поиск коллекций (Dynamic Collection Discovery)
    и выполняется ровно по тем, которые вернул адаптер.
    """
    _fresh_db()
    ...
    # Первоначальный запрос на target "initial_coll" возвращает 1 кошелек
    def fake_page(asset_type, target, offset, limit, network=None):
        if target == "initial_coll":
            return ([ActivityRecord("SOL_INIT", "buyer", "solana", "initial_coll", None, None)], False)
        if target == "trend_coll_1":
            return ([ActivityRecord("SOL_TREND", "buyer", "solana", "trend_coll_1", None, None)], False)
        return ([], False)

    fake_mkt.fetch_activity_page.side_effect = fake_page
    fake_mkt.get_trending_collections.return_value = ["initial_coll", "trend_coll_1", "trend_coll_2"]

    pipeline.MARKETPLACE_ADAPTERS["magic_eden"] = fake_mkt

    # Задаем цель target_wallets = 2, чтобы initial_coll не хватило, и запустился динамический поиск
    result = pipeline.run_job(
        platform="magic_eden", asset_type="nft", network="solana",
        target="initial_coll", target_wallets=2
    )

    assert result.unique_wallets_seen == 2
    assert result.targets_completed == 2  # initial_coll + trend_coll_1
    assert "trend_coll_1" in fake_mkt.fetch_activity_page.call_args_list[2][0]
...
if __name__ == "__main__":
    ...
    test_dynamic_collection_discovery()
